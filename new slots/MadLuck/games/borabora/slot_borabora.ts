class SlotBorabora extends SlotEnity implements ISlotEnity {
    ...
    public getResourseImg(callback: () => void): void {
        ...
        this.currProcNumLoadedObj = 100 / 10; //10 - число, загружаемых ресурсов, необходимо помнить что json нужно умножать на 2
        // обычный png или jpg на 1.
...
class MainSceneBorabora extends MainScene implements IMainScene {
    ...
    private v_alpha: number = 1;
    private direction_up: boolean = false;

    private ar_btn: Array<PIXI.Sprite>;
   ...
   /* private creatanim(_this): void {
        this.intervalId = setInterval(function (_this) {
            _this.updateanimbtn();
        }, 150,_this);
    }

    private updateanimbtn(): void {
        if (this.v_alpha >= 0.9) {
            this.direction_up = false;
            this.v_alpha -= 0.05;
        }
        else if (this.v_alpha <= 0.6) {
        ...
        this.v_alpha = +this.v_alpha.toPrecision(2);

        
        if (this.ar_btn) {
            var len: number = this.ar_btn.length;
            for (var i: number = 0; i < len; i++) {
                this.ar_btn[i].alpha = this.v_alpha;
            }
        }
        //console.log(this.v_alpha);
    }
    */
...
class HelpSceneBorabora extends HelpScene implements IHelpScene {
    constructor() {
...
class GambleSceneBorabora extends GambleScene implements IGambleScene {
...
class BonusSceneBorabora extends BonusScene implements IBonusScene {
...
//TO DO
var slotBorabora: SlotBorabora = new SlotBorabora();
